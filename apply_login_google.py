...
# -*- coding: utf-8 -*-
"""
Juara Travel CRM — Penampal automatik Login Google (Fasa 1)
============================================================
Skrip ini mengambil fail CRM ASAL awak dan menghasilkan fail baru
yang sudah lengkap dengan Login Google + role + Urus Pengguna.

CARA GUNA (dalam terminal):
    python3 apply_login_google.py NAMA_FAIL_ASAL.html

Contoh:
    python3 apply_login_google.py crm.html

Ia akan hasilkan:  index.html  (siap untuk deploy)
Fail asal awak TIDAK diubah.

⚠️ PENTING: Sebelum jalankan, isi email admin awak di bawah (baris ADMIN_EMAIL).
"""

import sys, re, os

# ============================================================
# ⚠️ ISI EMAIL GOOGLE AWAK DI SINI (admin pertama)
# ============================================================
ADMIN_EMAIL = "akmalakif5315@gmail.com"   # <-- TUKAR kepada email Google peribadi awak
# ============================================================
...
def must_replace(text, old, new, label):
    """Ganti sekali; gagal jika corak tak dijumpai (elak senyap-senyap silap)."""
    if old not in text:
        die(f"Tak jumpa bahagian untuk '{label}'. "
            f"Fail mungkin sudah diubah atau bukan fail asal yang betul.")
    if text.count(old) > 1:
        die(f"Corak '{label}' dijumpai lebih dari sekali — tak selamat untuk ganti automatik.")
    return text.replace(old, new)


def must_insert_before(text, anchor, insert, label):
    if anchor not in text:
        die(f"Tak jumpa penanda untuk '{label}'.")
    return text.replace(anchor, insert + anchor, 1)


def main():
    if len(sys.argv) < 2:
        die("Sila beri nama fail asal. Contoh: python3 apply_login_google.py crm.html")
    src = sys.argv[1]
    if not os.path.exists(src):
        die(f"Fail '{src}' tak wujud dalam folder ini.")
    if ADMIN_EMAIL == "emailanda@gmail.com":
        die("Awak belum tukar ADMIN_EMAIL dalam skrip ini. "
            "Buka apply_login_google.py, cari baris ADMIN_EMAIL, "
            "letak email Google awak, simpan, dan cuba lagi.")

    with open(src, "r", encoding="utf-8") as f:
        html = f.read()

    # Elak jalankan dua kali
    if "loginGoogle" in html or "BOOTSTRAP_ADMIN_EMAIL" in html:
        die("Fail ini nampaknya SUDAH ditampal (ada 'loginGoogle'). "
            "Jalankan skrip ini pada fail ASAL sahaja.")
    ...
    html = must_replace(
        ...
        f'const BOOTSTRAP_ADMIN_EMAIL = "{ADMIN_EMAIL}";',
        "init auth",
    )

    # ---- D1) Buang senarai password hardcoded ----
    users_block = re.search(r"const USERS=\[.*?\];", html, re.DOTALL)
    if not users_block:
        die("Tak jumpa senarai USERS untuk dibuang.")
    html = html.replace(users_block.group(0), "// (USERS hardcoded telah dibuang — login kini melalui Firebase Auth Google)")
    ...
    # ---- D2) Ganti login/doLoginUI/autoLogin/logout ----
    # Padam fungsi login() lama
    login_fn = re.search(r"function login\(\)\{.*?\n\}\n", html, re.DOTALL)
    if not login_fn:
        die("Tak jumpa fungsi login() lama.")
    html = html.replace(login_fn.group(0), "", 1)

    # Padam doLoginUI lama
    dologin_fn = re.search(r"function doLoginUI\(f\)\{.*?\n\}\n", html, re.DOTALL)
    if not dologin_fn:
        die("Tak jumpa fungsi doLoginUI() lama.")
    html = html.replace(dologin_fn.group(0), "", 1)

    # Padam autoLogin lama
    autologin_fn = re.search(r"function autoLogin\(\)\{.*?\n\}\n", html, re.DOTALL)
    if not autologin_fn:
        die("Tak jumpa fungsi autoLogin() lama.")
    html = html.replace(autologin_fn.group(0), "", 1)

    # Padam logout lama
    logout_fn = re.search(r"function logout\(\)\{.*?\n\}\n", html, re.DOTALL)
    if not logout_fn:
        die("Tak jumpa fungsi logout() lama.")
    html = html.replace(logout_fn.group(0), "", 1)
...
'''
    # Sisip fungsi auth baru sebelum fungsi subscribe()
    html = must_insert_before(html, "function subscribe(){", NEW_AUTH + "\n", "auth functions")

    # ---- D3) Kemas kini pendaftaran window.* ----
    html = must_replace(
        html,
    ...
    # ---- G) subscribe() langgan juara_users ----
    # Tambah subscribeUsers() sebelum penutup subscribe(): cari corak unsubA config assignments
    html = must_replace(
        html,
    ...
    # ---- F4) Logik Urus Pengguna (sisip sebelum generateScript) ----
    USERS_LOGIC = r'''
...
'''
    html = must_insert_before(html, "function generateScript(){", USERS_LOGIC + "\n", "users logic")

    # ---- F5) Daftar halaman users dalam nav() dan refresh() ----
    html = must_replace(
        html,
    ...
    # ---- E) Ganti skrin login dengan butang Google ----
    login_screen = re.search(r'<!-- LOGIN -->\s*<div id="lp">.*?</div>\s*</div>\s*</div>', html, re.DOTALL)
    if not login_screen:
        die("Tak jumpa skrin login lama untuk diganti.")
    NEW_LOGIN = '''<!-- LOGIN -->
<div id="lp">
  <div class="lc">
    <div class="l-logo">
    ...
    html = html.replace(login_screen.group(0), NEW_LOGIN, 1)

    # ---- F1) Item menu sidebar Urus Pengguna ----
    html = must_replace(
        html,
    ...
    # ---- F2) Halaman Urus Pengguna (sebelum <!-- SETTINGS -->) ----
    USERS_PAGE = '''      <!-- USERS -->
...
'''
    html = must_insert_before(html, "      <!-- SETTINGS -->", USERS_PAGE, "users page")

    # ---- F3) Modal tambah pengguna (sebelum toast) ----
    USER_MODAL = '''<div class="ov" id="m-user">
...
'''
    html = must_insert_before(html, '<div class="toast" id="toast"></div>', USER_MODAL, "user modal")

    # ---- H) Hadkan butang Padam lead kepada admin ----
    old_del = '''<button onclick="deleteLead('${l._id}','${(l.name||'').replace(/'/g,"\\\\'")}') " style="font-size:11px;padding:3px 8px;border:1px solid #fecaca;border-radius:4px;background:#fee2e2;cursor:pointer;color:#991b1b;margin-left:3px;font-weight:500">Padam</button>'''
    new_del = '''${isAdmin?`<button onclick="deleteLead('${l._id}','${(l.name||'').replace(/'/g,"\\\\'")}') " style="font-size:11px;padding:3px 8px;border:1px solid #fecaca;border-radius:4px;background:#fee2e2;cursor:pointer;color:#991b1b;margin-left:3px;font-weight:500">Padam</button>`:''}'''
    if old_del in html:
        html = html.replace(old_del, new_del, 1)
    else:
        print("⚠️  Nota: butang Padam lead tak dijumpai untuk dihadkan (mungkin format berbeza). Bahagian lain tetap diteruskan.")

    # ---- Buang panggilan autoLogin() di bahagian INIT ----
    ...
    print("   Admin pertama: " + ADMIN_EMAIL)
    print("\nLangkah seterusnya:")
    print("  1. Letak index.html dalam folder 'public' awak.")
    print("  2. Tambah URL .web.app baru ke Authorised domains (Firebase Console).")
    print("  3. firebase deploy")
    print("  4. Buka URL, login guna email admin di atas.\n")
